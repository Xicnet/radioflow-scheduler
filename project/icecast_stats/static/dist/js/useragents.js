/**
 * Example User Agents and their expected bowser objects.
 * Most of them where found at http://www.useragentstring.com/
 *
 * @see  test/test.js
 * @author hannes.diercks@jimdo.com
 */
var userAgents = {
  Chrome: {
    'Mozilla/5.0 (Linux; Android 8.0.0; Nexus 6P Build/OPR6.170623.013) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/60.0.3112.107 Mobile Safari/537.36': {
      osname: 'Android'
      , chrome: true
      , android: true
      , osversion: '8.0.0'
      , mobile: true
      , version: '60.0'
      , blink: true
      , a: true
    }
    , 'Mozilla/5.0 (Linux; Android 5.1.1; Nexus 9 Build/LMY48T) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/47.0.2526.83 Safari/537.36': {
      osname: 'Android'
      , chrome: true
      , android: true
      , osversion: '5.1.1'
      , tablet: true
      , version: '47.0'
      , blink: true
      , a: true
    }
    , 'Mozilla/5.0 (Linux; Android 4.4.2; Nexus 7 Build/KOT49H) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/32.0.1700.99 Safari/537.36': {
      osname: 'Android'
      , chrome: true
      , android: true
      , osversion: '4.4.2'
      , tablet: true
      , version: '32.0'
      , blink: true
      , a: true
    }
    , 'Mozilla/5.0 (Linux; Android 4.3; Galaxy Nexus Build/JWR66Y) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/32.0.1700.99 Mobile Safari/537.36': {
      osname: 'Android'
      , chrome: true
      , android: true
      , osversion: '4.3'
      , mobile: true
      , version: '32.0'
      , blink: true
      , a: true
    }
    , 'Mozilla/5.0 (Linux; Android 4.1; Galaxy Nexus Build/JRN84D) AppleWebKit/535.19 (KHTML, like Gecko) Chrome/18.0.1025.166 Mobile Safari/535.19': {
      osname: 'Android'
      , chrome: true
      , android: true
      , osversion: '4.1'
      , mobile: true
      , version: '18.0'
      , webkit: true
      , c: true
    }
    , 'Mozilla/5.0 (Linux; U; Android-4.0.3; en-us; Xoom Build/IML77) AppleWebKit/535.7 (KHTML, like Gecko) CrMo/16.0.912.75 Safari/535.7': {
      osname: 'Android'
      , chrome: true
      , android: true
      , osversion: '4.0.3'
      , tablet: true
      , version: '16.0'
      , webkit: true
      , c: true
    }
    , 'Mozilla/5.0 (Linux; U; Android-4.0.3; en-us; Galaxy Nexus Build/IML74K) AppleWebKit/535.7 (KHTML, like Gecko) CrMo/16.0.912.75 Mobile Safari/535.7': {
      osname: 'Android'
      , chrome: true
      , android: true
      , osversion: '4.0.3'
      , mobile: true
      , version: '16.0'
      , webkit: true
      , c: true
    }
    , 'Mozilla/5.0 (iPhone; U; CPU iPhone OS 5_1_1 like Mac OS X; en) AppleWebKit/534.46.0 (KHTML, like Gecko) CriOS/19.0.1084.60 Mobile/9B206 Safari/7534.48.3': {
      osname: 'iOS'
      , chrome: true
      , iphone: true
      , ios: true
      , osversion: '5.1.1'
      , mobile: true
      , version: '19.0'
      , webkit: true
      , c: true
    }
    , 'Mozilla/5.0 (iPad; U; CPU OS 5_1_1 like Mac OS X; en-us) AppleWebKit/534.46.0 (KHTML, like Gecko) CriOS/19.0.1084.60 Mobile/9B206 Safari/7534.48.3': {
      osname: 'iOS'
      , chrome: true
      , ipad: true
      , ios: true
      , osversion: '5.1.1'
      , tablet: true
      , version: '19.0'
      , webkit: true
      , c: true
    }
    , 'Mozilla/5.0 (Windows NT 6.2; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/30.0.1599.17 Safari/537.36': {
      osname: 'Windows'
      , chrome: true
      , version: '30.0'
      , windows: true
      , osversion: '8'
      , blink: true
      , a: true
    }
    , 'Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/29.0.1547.62 Safari/537.36': {
      osname: 'Windows'
      , chrome: true
      , version: '29.0'
      , windows: true
      , osversion: '7'
      , blink: true
      , a: true
    }
    , 'Mozilla/5.0 (X11; CrOS i686 4319.74.0) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/29.0.1547.57 Safari/537.36': {
      osname: 'Chrome OS'
      , chrome: true
      , chromeBook: true
      , version: '29.0'
      , chromeos: true
      , blink: true
      , a: true
    }
    , 'Mozilla/5.0 (Windows NT 6.2; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/29.0.1547.2 Safari/537.36': {
      osname: 'Windows'
      , chrome: true
      , version: '29.0'
      , windows: true
      , osversion: '8'
      , blink: true
      , a: true
    }
    , 'Mozilla/5.0 (Windows NT 6.1) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/28.0.1468.0 Safari/537.36': {
      osname: 'Windows'
      , chrome: true
      , version: '28.0'
      , windows: true
      , osversion: '7'
      , blink: true
      , a: true
    }
    , 'Mozilla/5.0 (Macintosh; Intel Mac OS X 10_6_7) AppleWebKit/534.24 (KHTML, like Gecko) Chrome/11.0.696.57 Safari/534.24': {
      osname: 'macOS'
      , chrome: true
      , version: '11.0'
      , mac: true
      , osversion: '10.6.7'
      , webkit: true
      , c: true
    }
    , 'Mozilla/5.0 (X11; U; Linux i686; en-US) AppleWebKit/534.3 (KHTML, like Gecko) Chrome/6.0.462.0 Safari/534.3': {
      osname: 'Linux'
      , chrome: true
      , version: '6.0'
      , linux: true
      , webkit: true
      , c: true
    }
    , 'Mozilla/5.0 (iPhone; CPU iPhone OS 9_3_1 like Mac OS X) AppleWebKit/601.1 (KHTML, like Gecko) CriOS/50.0.2661.95 Mobile/13E238 Safari/601.1.46': {
      osname: 'iOS'
      , chrome: true
      , version: '50.0'
      , osversion: '9.3.1'
      , webkit: true
      , ios: true
      , iphone: true
      , mobile: true
      , a: true
    }
    , 'Mozilla/5.0 (Linux; Android 5.0.2; SM-T705 Build/LRX22G) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/49.0.2623.105 Safari/537.36': {
      osname: 'Android'
      , chrome: true
      , android: true
      , version: '49.0'
      , osversion: '5.0.2'
      , blink: true
      , a: true
      , tablet: true
    }
    , 'Mozilla/5.0 (Linux; Android 6.0.99; Build/NPC91K) AppleWebKit/537.36(KHTML, like Gecko) Chrome/50.0.2661.86 Safari/537.36': {
      osname: 'Android'
      , chrome: true
      , android: true
      , tablet: true
      , osversion: '6.0.99'
      , version: '50.0'
      , blink: true
      , a: true
    }
  }
  , 'Amazon Silk': {
    'Mozilla/5.0 (Linux; U; Android 4.0.3; en-us; KFTT Build/IML74K) AppleWebKit/535.19 (KHTML, like Gecko) Silk/3.4 Mobile Safari/535.19 Silk-Accelerated=true': {
      osname: 'Android'
      , silk: true
      , webkit: true
      , android: true
      , osversion: '4.0.3'
      , tablet: true
      , version: '3.4'
      , x: true
    }
    , 'Mozilla/5.0 (Linux; U; en-us; KFTT Build/IML74K) AppleWebKit/535.19 (KHTML, like Gecko) Silk/3.4 Safari/535.19 Silk-Accelerated=true': {
      osname: 'Android'
      , silk: true
      , webkit: true
      , android: true
      , tablet: true
      , version: '3.4'
      , x: true
    }
    , 'Mozilla/5.0 (Linux; U; Android 2.3.4; en-us; Silk/1.0.13.81_10003810) AppleWebKit/533.1 (KHTML, like Gecko) Version/4.0 Mobile Safari/533.1 Silk-Accelerated=true': {
      osname: 'Android'
      , silk: true
      , webkit: true
      , android: true
      , osversion: '2.3.4'
      , tablet: true
      , version: '1.0'
      , x: true
    }
    , 'Mozilla/5.0 (Macintosh; U; Intel Mac OS X 10_6_3; en-us; Silk/1.0.22.153_10033210) AppleWebKit/533.16 (KHTML, like Gecko) Version/5.0 Safari/533.16 Silk-Accelerated=true': {
      osname: 'Android'
      , silk: true
      , webkit: true
      , android: true
      , tablet: true
      , version: '1.0'
      , x: true
    }
  }
  , Opera: {
    'Mozilla/5.0 (Macintosh; Intel Mac OS X 10_11_6) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/60.0.3112.78 Safari/537.36 OPR/47.0.2631.55': {
      osname: 'macOS'
      , opera: true
      , blink: true
      , mac: true
      , osversion: '10.11.6'
      , version: '47.0'
      , a: true
    }
    , 'Mozilla/5.0 (Linux; Android 8.0; Nexus 6P Build/OPR6.170623.013) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/55.0.2883.91 Mobile Safari/537.36 OPR/42.7.2246.114996': {
      osname: 'Android'
      , opera: true
      , blink: true
      , android: true
      , osversion: '8.0'
      , mobile: true
      , version: '42.7'
      , a: true
    }
    , 'Mozilla/5.0 (Linux; Android 4.4.2; Nexus 7 Build/KOT49H) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/32.0.1700.72 Safari/537.36 OPR/19.0.1340.69721': {
      osname: 'Android'
      , opera: true
      , blink: true
      , android: true
      , osversion: '4.4.2'
      , tablet: true
      , version: '19.0'
      , a: true
    }
    , 'Mozilla/5.0 (Linux; Android 4.3; Galaxy Nexus Build/JWR66Y) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/32.0.1700.72 Mobile Safari/537.36 OPR/19.0.1340.69721': {
      osname: 'Android'
      , opera: true
      , blink: true
      , android: true
      , osversion: '4.3'
      , mobile: true
      , version: '19.0'
      , a: true
    }
    , 'Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/28.0.1500.52 Safari/537.36 OPR/15.0.1147.100': {
      osname: 'Windows'
      , opera: true
      , windows: true
      , osversion: '7'
      , blink: true
      , version: '15.0'
      , a: true
    }
    , 'Opera/9.80 (Android 4.3; Linux; Opera Mobi/ADR-1309251116) Presto/2.11.355 Version/12.10': {
      osname: 'Android'
      , opera: true
      , version: '12.10'
      , android: true
      , osversion: '4.3'
      , mobile: true
      , a: true
    }
    , 'Opera/9.80 (Android 4.4.2; Linux; Opera Tablet/ADR-1309251116) Presto/2.11.355 Version/12.10': {
      osname: 'Android'
      , opera: true
      , version: '12.10'
      , android: true
      , osversion: '4.4.2'
      , tablet: true
      , a: true
    }
    , 'Opera/9.80 (Windows NT 6.0) Presto/2.12.388 Version/12.14': {
      osname: 'Windows'
      , opera: true
      , version: '12.14'
      , windows: true
      , osversion: 'Vista'
      , a: true
    }
    , 'Mozilla/5.0 (Windows NT 6.0; rv:2.0) Gecko/20100101 Firefox/4.0 Opera 12.14': {
      osname: 'Windows'
      , opera: true
      , version: '12.14'
      , windows: true
      , osversion: 'Vista'
      , a: true
    }
    , 'Mozilla/5.0 (compatible; MSIE 9.0; Windows NT 6.0) Opera 12.14': {
      osname: 'Windows'
      , opera: true
      , version: '12.14'
      , windows: true
      , osversion: 'Vista'
      , a: true
    }
    , 'Opera/12.80 (Windows NT 5.1; U; en) Presto/2.10.289 Version/12.02': {
      osname: 'Windows'
      , opera: true
      , version: '12.02'
      , windows: true
      , osversion: 'XP'
      , a: true
    }
    , 'Opera/9.80 (X11; Linux i686; U; es-ES) Presto/2.8.131 Version/11.11': {
      osname: 'Linux'
      , opera: true
      , version: '11.11'
      , linux: true
      , a: true
    }
    , 'Opera/9.80 (Macintosh; Intel Mac OS X 10.6.7; U; en) Presto/2.7.62 Version/11.01': {
      osname: 'macOS'
      , opera: true
      , version: '11.01'
      , mac: true
      , osversion: '10.6.7'
      , a: true
    }
    , 'Opera/9.80 (Windows NT 5.2; U; zh-cn) Presto/2.6.30 Version/10.63': {
      osname: 'Windows'
      , opera: true
      , version: '10.63'
      , windows: true
      , osversion: '2003'
      , a: true
    }
    , 'Opera/9.80 (X11; Linux i686; U; it) Presto/2.5.24 Version/10.54': {
      osname: 'Linux'
      , opera: true
      , version: '10.54'
      , linux: true
      , a: true
    }
    , 'Opera/9.70 (Linux ppc64 ; U; en) Presto/2.2.1': {
      osname: 'Linux'
      , opera: true
      , version: '9.70'
      , linux: true
      , c: true
    }
    , 'Opera/9.63 (X11; Linux i686)': {
      osname: 'Linux'
      , opera: true
      , version: '9.63'
      , linux: true
      , c: true
    }
    , 'Mozilla/5.0 (X11; Linux i686; U; en) Opera 8.52': {
      osname: 'Linux'
      , opera: true
      , version: '8.52'
      , linux: true
      , c: true
    }
    , 'Mozilla/5.0 (iPod touch; CPU iPhone OS 9_3_4 like Mac OS X) AppleWebKit/601.1.46 (KHTML, like Gecko) OPiOS/14.0.0.104835 Mobile/13G35 Safari/9537.53': {
      osname: 'iOS'
      , opera: true
      , version: '14.0'
      , mobile: true
      , ipod: true
      , ios: true
      , a: true
      , osversion: '9.3.4'
      , webkit: true
    }
    , 'Mozilla/5.0 (Linux; U; Android 6.0; R1 HD Build/MRA58K; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/52.0.2743.98 Mobile Safari/537.36 OPR/18.0.2254.106542': {
      osname: 'Android'
      , opera: true
      , version: '18.0'
      , blink: true
      , mobile: true
      , android: true
      , a: true
      , osversion: '6.0'
    }
  }
  , 'Opera Coast': {
    'Mozilla/5.0 (iPhone; CPU iPhone OS 9_3_1 like Mac OS X) AppleWebKit/601.1.46 (KHTML, like Gecko) Coast/5.02.99991 Mobile/13E238 Safari/7534.48.3': {
      osname: 'iOS'
      , coast: true
      , version: '5.02'
      , osversion: '9.3.1'
      , iphone: true
      , ios: true
      , mobile: true
      , a: true
      , webkit: true
    }
  }
  , 'Yandex Browser': {
    'Mozilla/5.0 (Macintosh; Intel Mac OS X 10_10_3) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/41.0.2272.118 YaBrowser/15.4.2272.3420 (beta) Yowser/2.0 Safari/537.36': {
      osname: 'macOS'
      , yandexbrowser: true
      , blink: true
      , version: '15.4'
      , mac: true
      , osversion: '10.10.3'
      , a: true
    }
    , 'Mozilla/5.0 (Linux; Android 5.1.1; Nexus 5 Build/LMY48B) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/41.0.2272.118 YaBrowser/15.4.2272.3608.00 Mobile Safari/537.36': {
      osname: 'Android'
      , yandexbrowser: true
      , android: true
      , osversion: '5.1.1'
      , mobile: true
      , version: '15.4'
      , blink: true
      , a: true
    }
  }
  , Safari: {
    'Mozilla/5.0 (Macintosh; Intel Mac OS X 10_6_8) AppleWebKit/537.13+ (KHTML, like Gecko) Version/5.1.7 Safari/534.57.2': {
      osname: 'macOS'
      , safari: true
      , version: '5.1'
      , mac: true
      , osversion: '10.6.8'
      , webkit: true
      , c: true
    }
    , 'Mozilla/5.0 (Macintosh; U; Intel Mac OS X 10_6_7; en-us) AppleWebKit/533.21.1 (KHTML, like Gecko) Version/5.0.5 Safari/533.21.1': {
      osname: 'macOS'
      , safari: true
      , version: '5.0'
      , mac: true
      , osversion: '10.6.7'
      , webkit: true
      , c: true
    }
    , 'Mozilla/5.0 (Windows; U; Windows NT 5.1; ru-RU) AppleWebKit/533.18.1 (KHTML, like Gecko) Version/5.0.2 Safari/533.18.5': {
      osname: 'Windows'
      , safari: true
      , version: '5.0'
      , windows: true
      , osversion: 'XP'
      , webkit: true
      , c: true
    }
    , 'Mozilla/5.0 (X11; U; Linux x86_64; en-us) AppleWebKit/531.2+ (KHTML, like Gecko) Version/5.0 Safari/531.2+': {
      osname: 'Linux'
      , safari: true
      , version: '5.0'
      , linux: true
      , webkit: true
      , c: true
    }
    , 'Mozilla/5.0 (Windows; U; Windows NT 5.0; en-en) AppleWebKit/533.16 (KHTML, like Gecko) Version/4.1 Safari/533.16': {
      osname: 'Windows'
      , safari: true
      , version: '4.1'
      , windows: true
      , osversion: '2000'
      , webkit: true
      , c: true
    }
    , 'Mozilla/5.0 (Macintosh; U; PPC Mac OS X 10_6_1; en_GB, en_US) AppleWebKit/531.21.10 (KHTML, like Gecko) Version/4.0.4 Safari/531.21.10': {
      osname: 'macOS'
      , safari: true
      , version: '4.0'
      , mac: true
      , osversion: '10.6.1'
      , webkit: true
      , c: true
    }
    , 'Mozilla/5.0 (Macintosh; U; Intel Mac OS X 10_5_7; de-de) AppleWebKit/525.28.3 (KHTML, like Gecko) Version/3.2.3 Safari/525.28.3': {
      osname: 'macOS'
      , safari: true
      , version: '3.2'
      , mac: true
      , osversion: '10.5.7'
      , webkit: true
      , c: true
    }
    , 'Mozilla/5.0 (iPhone; CPU iPhone OS 7_0_4 like Mac OS X) AppleWebKit/537.51.1 (KHTML, like Gecko) Mobile/11B554a': {
      osname: 'iOS'
      , ios: true
      , osversion: '7.0.4'
      , iphone: true
      , mobile: true
      , webkit: true
      , safari: true
      , a: true
    }
    , 'Mozilla/5.0 (iPhone Simulator; U; CPU iPhone OS 4_3_2 like Mac OS X; en-us) AppleWebKit/533.17.9 (KHTML, like Gecko) Version/5.0.2 Mobile/8H7 Safari/6533.18.5': {
      osname: 'iOS'
      , ios: true
      , osversion: '4.3.2'
      , version: '5.0'
      , iphone: true
      , mobile: true
      , safari: true
      , webkit: true
      , c: true
    }
    , 'Mozilla/5.0 (iPhone; U; CPU like Mac OS X; en) AppleWebKit/420+ (KHTML, like Gecko) Version/3.0 Mobile/1A543a Safari/419.3': {
      osname: 'iOS'
      , ios: true
      , version: '3.0'
      , iphone: true
      , mobile: true
      , webkit: true
      , safari: true
      , c: true
    }
    , 'Mozilla/5.0 (iPhone; U; CPU iPhone OS 4_1 like Mac OS X; en-us) AppleWebKit/532.9 (KHTML, like Gecko) Version/4.0.5 Mobile/8B5097d Safari/6531.22.7': {
      osname: 'iOS'
      , ios: true
      , osversion: '4.1'
      , version: '4.0'
      , iphone: true
      , mobile: true
      , webkit: true
      , safari: true
      , c: true
    }
    , 'Mozilla/5.0 (Macintosh; U; PPC Mac OS X 10_5_2; en-gb) AppleWebKit/526+ (KHTML, like Gecko) Version/3.1 iPhone': {
      osname: 'iOS'
      , ios: true
      , version: '3.1'
      , iphone: true
      , mobile: true
      , webkit: true
      , safari: true
      , c: true
    }
    , 'Mozilla/5.0 (iPhone; CPU iPhone OS 9_0 like Mac OS X) AppleWebKit/601.1.46 (KHTML, like Gecko) Version/9.0 Mobile/13A344 Safari/601.1': {
      osname: 'iOS'
      , ios: true
      , osversion: '9.0'
      , version: '9.0'
      , iphone: true
      , mobile: true
      , webkit: true
      , safari: true
      , a: true
    }
    , 'Mozilla/5.0 (iPad; CPU OS 7_0_4 like Mac OS X) AppleWebKit/537.51.1 (KHTML, like Gecko) Version/7.0 Mobile/11B554a Safari/9537.53': {
      osname: 'iOS'
      , ios: true
      , osversion: '7.0.4'
      , version: '7.0'
      , ipad: true
      , tablet: true
      , webkit: true
      , safari: true
      , a: true
    }
    , 'Mozilla/5.0 (iPad; CPU OS 6_0 like Mac OS X) AppleWebKit/536.26 (KHTML, like Gecko) Version/6.0 Mobile/10A5355d Safari/8536.25': {
      osname: 'iOS'
      , ios: true
      , osversion: '6.0'
      , version: '6.0'
      , ipad: true
      , tablet: true
      , webkit: true
      , safari: true
      , a: true
    }
    , 'Mozilla/5.0 (iPad; CPU OS 5_1 like Mac OS X) AppleWebKit/534.46 (KHTML, like Gecko ) Version/5.1 Mobile/9B176 Safari/7534.48.3': {
      osname: 'iOS'
      , ios: true
      , osversion: '5.1'
      , version: '5.1'
      , ipad: true
      , tablet: true
      , webkit: true
      , safari: true
      , c: true
    }
    , 'Mozilla/5.0 (iPad; U; CPU OS 4_3_2 like Mac OS X; en-us) AppleWebKit/533.17.9 (KHTML, like Gecko) Version/5.0.2 Mobile/8H7 Safari/6533.18.5': {
      osname: 'iOS'
      , ios: true
      , osversion: '4.3.2'
      , version: '5.0'
      , ipad: true
      , tablet: true
      , webkit: true
      , safari: true
      , c: true
    }
    , 'Mozilla/5.0 (iPad; U; CPU OS 3_2 like Mac OS X; es-es) AppleWebKit/531.21.10 (KHTML, like Gecko) Version/4.0.4 Mobile/7B360 Safari/531.21.10': {
      osname: 'iOS'
      , ios: true
      , osversion: '3.2'
      , version: '4.0'
      , ipad: true
      , tablet: true
      , webkit: true
      , safari: true
      , c: true
    }
    , 'Mozilla/5.0 (iPod touch; CPU iPhone OS 7_0_3 like Mac OS X) AppleWebKit/537.51.1 (KHTML, like Gecko) Version/7.0 Mobile/11B511 Safari/9537.53': {
      osname: 'iOS'
      , ios: true
      , osversion: '7.0.3'
      , version: '7.0'
      , ipod: true
      , mobile: true
      , webkit: true
      , safari: true
      , a: true
    }
    , 'Mozilla/5.0 (iPod; CPU iPhone OS 5_1 like Mac OS X) AppleWebKit/534.46 (KHTML, like Gecko) Version/5.1 Mobile/9B176 Safari/7534.48.3': {
      osname: 'iOS'
      , ios: true
      , osversion: '5.1'
      , version: '5.1'
      , ipod: true
      , mobile: true
      , webkit: true
      , safari: true
      , c: true
    }
    , 'Mozilla/5.0 (iPod; U; CPU iPhone OS 4_3_3 like Mac OS X; en-us) AppleWebKit/533.17.9 (KHTML, like Gecko) Version/5.0.2 Mobile/8J2 Safari/6533.18.5': {
      osname: 'iOS'
      , ios: true
      , osversion: '4.3.3'
      , version: '5.0'
      , ipod: true
      , mobile: true
      , webkit: true
      , safari: true
      , c: true
    }
    , 'Mozilla/5.0 (iPod; U; CPU like Mac OS X; en) AppleWebKit/420.1 (KHTML, like Gecko) Version/3.0 Mobile/4A93 Safari/419.3': {
      osname: 'iOS'
      , ios: true
      , version: '3.0'
      , ipod: true
      , mobile: true
      , webkit: true
      , safari: true
      , c: true
    }
    , 'Mozilla/5.0 (iPhone; CPU iPhone OS 9_3_1 like Mac OS X) AppleWebKit/601.1.46 (KHTML, like Gecko) Version/9.0 Mobile/13E238 Safari/601.1': {
      osname: 'iOS'
      , ios: true
      , iphone: true
      , safari: true
      , version: '9.0'
      , mobile: true
      , osversion: '9.3.1'
      , a: true
      , webkit: true
    }
    , 'Mozilla/5.0 (iPhone; CPU iPhone OS 11_0 like Mac OS X) AppleWebKit/604.1.21 (KHTML, like Gecko) Version/10.0 Mobile/15A5278f Safari/602.1': {
      osname: 'iOS'
      , version: '10.0'
      , iphone: true
      , safari: true
      , ios: true
      , mobile: true
      , osversion: '11.0'
      , a: true
      , webkit: true
    }
  }
  , 'Internet Explorer': {
    'Mozilla/5.0 (Windows NT 6.3; Win64; x64; Trident/7.0; MAARJS; rv:11.0) like Gecko': {
      osname: 'Windows'
      , msie: true
      , version: '11.0'
      , windows: true
      , osversion: '8.1'
      , a: true
    }
    , 'Mozilla/5.0 (Windows NT 6.3; WOW64; Trident/7.0; Touch; rv:11.0) like Gecko': {
      osname: 'Windows'
      , msie: true
      , version: '11.0'
      , windows: true
      , osversion: '8.1'
      , a: true
    }
    , 'Mozilla/5.0 (compatible; MSIE 10.0; Windows NT 6.2; Trident/6.0; ARM; Touch; WPDesktop)': {
      osname: 'Windows'
      , msie: true
      , version: '10.0'
      , windows: true
      , osversion: '8'
      , a: true
    }
    , 'Mozilla/5.0 (Windows NT 6.3; WOW64; Trident/7.0; .NET4.0E; .NET4.0C; Media Center PC 6.0; rv:11.0) like Gecko': {
      osname: 'Windows'
      , msie: true
      , version: '11.0'
      , windows: true
      , osversion: '8.1'
      , a: true
    }
    , 'Mozilla/5.0 (compatible; MSIE 10.6; Windows NT 6.1; Trident/5.0; InfoPath.2; SLCC1; .NET CLR 3.0.4506.2152; .NET CLR 3.5.30729; .NET CLR 2.0.50727) 3gpp-gba UNTRUSTED/1.0': {
      osname: 'Windows'
      , msie: true
      , version: '10.6'
      , windows: true
      , osversion: '7'
      , a: true
    }
    , 'Mozilla/5.0 (compatible; MSIE 10.0; Windows NT 6.1; Trident/4.0; InfoPath.2; SV1; .NET CLR 2.0.50727; WOW64)': {
      osname: 'Windows'
      , msie: true
      , version: '10.0'
      , windows: true
      , osversion: '7'
      , a: true
    }
    , 'Mozilla/5.0 (compatible; MSIE 9.0; Windows NT 6.1; Win64; x64; Trident/5.0; .NET CLR 3.5.30729; .NET CLR 3.0.30729; .NET CLR 2.0.50727; Media Center PC 6.0)': {
      osname: 'Windows'
      , msie: true
      , version: '9.0'
      , windows: true
      , osversion: '7'
      , c: true
    }
    , 'Mozilla/4.0 (compatible; MSIE 8.0; Windows NT 6.1; Trident/5.0; SLCC2; .NET CLR 2.0.50727; .NET CLR 3.5.30729; .NET CLR 3.0.30729; Media Center PC 6.0; .NET4.0C)': {
      osname: 'Windows'
      , msie: true
      , version: '8.0'
      , windows: true
      , osversion: '7'
      , c: true
    }
    , 'Mozilla/4.0 (compatible; MSIE 7.0b; Windows NT 5.1; .NET CLR 1.1.4322; InfoPath.1; .NET CLR 2.0.50727)': {
      osname: 'Windows'
      , msie: true
      , version: '7.0'
      , windows: true
      , osversion: 'XP'
      , c: true
    }
    , 'Mozilla/4.0 (compatible; MSIE 6.1; Windows XP; .NET CLR 1.1.4322; .NET CLR 2.0.50727)': {
      osname: 'Windows'
      , msie: true
      , version: '6.1'
      , windows: true
      , osversion: 'XP'
      , c: true
    }
    , 'Mozilla/4.0 (Compatible; Windows NT 5.1; MSIE 6.0) (compatible; MSIE 6.0; Windows NT 5.1; .NET CLR 1.1.4322; .NET CLR 2.0.50727)': {
      osname: 'Windows'
      , msie: true
      , version: '6.0'
      , windows: true
      , osversion: 'XP'
      , c: true
    }
    , 'Mozilla/4.0 (compatible; MSIE 5.01; Windows NT)': {
      osname: 'Windows'
      , msie: true
      , version: '5.01'
      , windows: true
      , osversion: 'NT'
      , c: true
    }
    , 'Mozilla/5.0 (compatible; MSIE 10.0; Windows NT 6.2; Trident/6.0; Xbox; Xbox One)': {
      osname: 'Xbox'
      , xbox: true
      , msie: true
      , version: '10.0'
      , a: true
    }
    , 'Mozilla/5.0 (Windows NT 6.3; ARM; Trident/7.0; Touch; rv:11.0) like Gecko': {
      osname: 'Windows'
      , msie: true
      , version: '11.0'
      , a: true
      , windows: true
      , osversion: '8.1'
    }
    , 'Mozilla/5.0 (Windows NT 10.0; WOW64; Trident/7.0; Touch; LCTE; rv:11.0)': {
      osname: 'Windows'
      , msie: true
      , version: '11.0'
      , a: true
      , windows: true
      , osversion: '10'
    }
  }
  , 'Microsoft Edge': {
    'Mozilla/5.0 (Windows NT 10.0; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/39.0.2171.71 Safari/537.36 Edge/12.0': {
      osname: 'Windows'
      , msedge: true
      , version: '12.0'
      , windows: true
      , osversion: '10'
      , a: true
    }
    , 'Mozilla/5.0 (iPhone; CPU iPhone OS 10_3_2 like Mac OS X) AppleWebKit/603.2.4 (KHTML, like Gecko) Mobile/14F89 Safari/603.2.4 EdgiOS/41.1.35.1': {
      osname: 'iOS'
      , msedge: true
      , version: '41.1'
      , ios: true
      , iphone: true
      , mobile: true
      , osversion: '10.3.2'
      , a: true
    }
    , 'Mozilla/5.0 (Linux; Android 8.0; Pixel XL Build/OPP3.170518.006) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/58.0.3029.0 Mobile Safari/537.36 EdgA/41.1.35.1': {
      osname: 'Android'
      , msedge: true
      , version: '41.1'
      , android: true
      , mobile: true
      , osversion: '8.0'
      , a: true
    }
  }
  , Firefox: {
    'Mozilla/5.0 (Mobile; rv:26.0) Gecko/26.0 Firefox/26.0': {
      osname: 'Firefox OS'
      , gecko: true
      , firefox: true
      , version: '26.0'
      , mobile: true
      , firefoxos: true
      , a: true
    }
    , 'Mozilla/5.0 (Tablet; rv:26.0) Gecko/26.0 Firefox/26.0': {
      osname: 'Firefox OS'
      , gecko: true
      , firefox: true
      , version: '26.0'
      , tablet: true
      , firefoxos: true
      , a: true
    }
    , 'Mozilla/5.0 (Mobile; LG-D300; rv:18.1) Gecko/18.1 Firefox/18.1': {
      osname: 'Firefox OS'
      , gecko: true
      , firefox: true
      , version: '18.1'
      , mobile: true
      , firefoxos: true
      , c: true
    }
    , 'Mozilla/5.0 (Android; Mobile; rv:27.0) Gecko/27.0 Firefox/27.0': {
      osname: 'Android'
      , gecko: true
      , firefox: true
      , version: '27.0'
      , mobile: true
      , android: true
      , a: true
    }
    , 'Mozilla/5.0 (Android; Tablet; rv:26.0) Gecko/26.0 Firefox/26.0': {
      osname: 'Android'
      , gecko: true
      , firefox: true
      , version: '26.0'
      , tablet: true
      , android: true
      , a: true
    }
    , 'Mozilla/5.0 (Windows NT 6.1; Win64; x64; rv:25.0) Gecko/20100101 Firefox/25.0': {
      osname: 'Windows'
      , gecko: true
      , firefox: true
      , version: '25.0'
      , windows: true
      , osversion: '7'
      , a: true
    }
    , 'Mozilla/5.0 (Macintosh; Intel Mac OS X 10.8; rv:24.0) Gecko/20100101 Firefox/24.0': {
      osname: 'macOS'
      , gecko: true
      , firefox: true
      , version: '24.0'
      , mac: true
      , osversion: '10.8'
      , a: true
    }
    , 'Mozilla/5.0 (X11; Linux i686; rv:21.0) Gecko/20100101 Firefox/21.0': {
      osname: 'Linux'
      , gecko: true
      , firefox: true
      , version: '21.0'
      , linux: true
      , a: true
    }
    , 'Mozilla/5.0 (X11; Linux x86_64; rv:17.0) Gecko/20121202 Firefox/17.0 Iceweasel/17.0.1': {
      osname: 'Linux'
      , gecko: true
      , firefox: true
      , version: '17.0'
      , linux: true
      , c: true
    }
    , 'Mozilla/5.0 (X11; Linux x86_64; rv:15.0) Gecko/20120724 Debian Iceweasel/15.0': {
      osname: 'Linux'
      , gecko: true
      , firefox: true
      , version: '15.0'
      , linux: true
      , c: true
    }
    , 'Mozilla/5.0 (Windows NT 6.2; WOW64; rv:15.0) Gecko/20120910144328 Firefox/15.0.2': {
      osname: 'Windows'
      , gecko: true
      , firefox: true
      , version: '15.0'
      , windows: true
      , osversion: '8'
      , c: true
    }
    , 'Mozilla/5.0 (Windows; U; Windows NT 6.1; WOW64; en-US; rv:2.0.4) Gecko/20120718 AskTbAVR-IDW/3.12.5.17700 Firefox/14.0.1': {
      osname: 'Windows'
      , gecko: true
      , firefox: true
      , version: '14.0'
      , windows: true
      , osversion: '7'
      , c: true
    }
    , 'Mozilla/5.0 (Windows NT 5.1; rv:6.0) Gecko/20100101 Firefox/6.0 FirePHP/0.6': {
      osname: 'Windows'
      , gecko: true
      , firefox: true
      , version: '6.0'
      , windows: true
      , osversion: 'XP'
      , c: true
    }
    , 'Mozilla/5.0 (X11; Linux x86_64; rv:2.2a1pre) Gecko/20100101 Firefox/4.2a1pre': {
      osname: 'Linux'
      , gecko: true
      , firefox: true
      , version: '4.2'
      , linux: true
      , c: true
    }
    , 'Mozilla/5.0 (Macintosh; Intel Mac OS X 10.6; rv:2.0) Gecko/20100101 Firefox/4.0': {
      osname: 'macOS'
      , gecko: true
      , firefox: true
      , version: '4.0'
      , mac: true
      , osversion: '10.6'
      , c: true
    }
    , 'Mozilla/5.0 (Windows; U; Windows NT 6.1; en-US; rv:1.9.2b1) Gecko/20091014 Firefox/3.6b1 GTB5': {
      osname: 'Windows'
      , gecko: true
      , firefox: true
      , version: '3.6'
      , windows: true
      , osversion: '7'
      , c: true
    }
    , 'Mozilla/5.0 (Windows; U; Windows NT 6.0; de; rv:1.9.0.15) Gecko/2009101601 Firefox 2.1 (.NET CLR 3.5.30729)': {
      osname: 'Windows'
      , gecko: true
      , firefox: true
      , version: '2.1'
      , windows: true
      , osversion: 'Vista'
      , c: true
    }
    , 'Mozilla/5.0 (X11; U; Linux i686; en-US; rv:1.8.0.7) Gecko/20061014 Firefox/1.5.0.7': {
      osname: 'Linux'
      , gecko: true
      , firefox: true
      , version: '1.5'
      , linux: true
      , c: true
    }
    , 'Mozilla/5.0 (iPhone; CPU iPhone OS 9_3_1 like Mac OS X) AppleWebKit/601.1.46 (KHTML, like Gecko) FxiOS/3.0 Mobile/13E238 Safari/601.1.46': {
      osname: 'iOS'
      , firefox: true
      , version: '3.0'
      , osversion: '9.3.1'
      , ios: true
      , iphone: true
      , mobile: true
      , a: true
      , webkit: true
    }
  }
  , SeaMonkey: {
    'Mozilla/5.0 (Windows NT 5.2; rv:10.0.1) Gecko/20100101 Firefox/10.0.1 SeaMonkey/2.7.1': {
      osname: 'Windows'
      , gecko: true
      , seamonkey: true
      , version: '2.7'
      , windows: true
      , osversion: '2003'
      , x: true
    }
    , 'Mozilla/5.0 (Macintosh; Intel Mac OS X 10.5; rv:10.0.1) Gecko/20100101 Firefox/10.0.1 SeaMonkey/2.7.1': {
      osname: 'macOS'
      , gecko: true
      , seamonkey: true
      , version: '2.7'
      , mac: true
      , osversion: '10.5'
      , x: true
    }
    , 'Mozilla/5.0 (X11; Linux i686; rv:10.0.1) Gecko/20100101 Firefox/10.0.1 SeaMonkey/2.7.1': {
      osname: 'Linux'
      , gecko: true
      , seamonkey: true
      , version: '2.7'
      , linux: true
      , x: true
    }
  }
  , BlackBerry: {
    'Mozilla/5.0 (BB10; Touch) AppleWebKit/537.10+ (KHTML, like Gecko) Version/10.1.0.4633 Mobile Safari/537.10+': {
      osname: 'BlackBerry OS'
      , blackberry: true
      , version: '10.1'
      , webkit: true
      , mobile: true
      , a: true
    }
    , 'Mozilla/5.0 (BB10; Kbd) AppleWebKit/537.10+ (KHTML, like Gecko) Version/10.1.0.1429 Mobile Safari/537.10+': {
      osname: 'BlackBerry OS'
      , blackberry: true
      , version: '10.1'
      , webkit: true
      , mobile: true
      , a: true
    }
    , 'Mozilla/5.0 (PlayBook; U; RIM Tablet OS 2.1.0; en-US) AppleWebKit/536.2+ (KHTML, like Gecko) Version/7.2.1.0 Safari/536.2+': {
      osname: 'BlackBerry OS'
      , blackberry: true
      , osversion: '2.1.0'
      , version: '7.2'
      , webkit: true
      , tablet: true
      , x: true
    }
    , 'Mozilla/5.0 (PlayBook; U; RIM Tablet OS 1.0.0; en-US) AppleWebKit/534.11 (KHTML, like Gecko) Version/7.1.0.7 Safari/534.11': {
      osname: 'BlackBerry OS'
      , blackberry: true
      , osversion: '1.0.0'
      , version: '7.1'
      , webkit: true
      , tablet: true
      , x: true
    }
    , 'Mozilla/5.0 (BlackBerry; U; BlackBerry 9900; en) AppleWebKit/534.11+ (KHTML, like Gecko) Version/7.1.0.346 Mobile Safari/534.11+': {
      osname: 'BlackBerry OS'
      , blackberry: true
      , version: '7.1'
      , webkit: true
      , mobile: true
      , x: true
    }
    , 'Mozilla/5.0 (BlackBerry; U; BlackBerry 9360; en) AppleWebKit/534.11+ (KHTML, like Gecko) Version/7.0.0.530 Mobile Safari/534.11+': {
      osname: 'BlackBerry OS'
      , blackberry: true
      , version: '7.0'
      , webkit: true
      , mobile: true
      , x: true
    }
    , 'Mozilla/5.0 (BlackBerry; U; BlackBerry 9800; en-US) AppleWebKit/534.8+ (KHTML, like Gecko) Version/6.0.0.450 Mobile Safari/534.8+': {
      osname: 'BlackBerry OS'
      , blackberry: true
      , version: '6.0'
      , webkit: true
      , mobile: true
      , x: true
    }
    , 'BlackBerry9800/5.0.0.862 Profile/MIDP-2.1 Configuration/CLDC-1.1 VendorID/331 UNTRUSTED/1.0 3gpp-gba': {
      osname: 'BlackBerry OS'
      , blackberry: true
      , version: '5.0'
      , mobile: true
      , x: true
    }
    , 'BlackBerry8320/4.5.0.52 Profile/MIDP-2.0 Configuration/CLDC-1.1 VendorID/179': {
      osname: 'BlackBerry OS'
      , blackberry: true
      , version: '4.5'
      , mobile: true
      , x: true
    }
    , 'BlackBerry8700/4.1.0 Profile/MIDP-2.0 Configuration/CLDC-1.1 VendorID/179': {
      osname: 'BlackBerry OS'
      , blackberry: true
      , version: '4.1'
      , mobile: true
      , x: true
    }
  }
  , 'Windows Phone': {
    'Mozilla/5.0 (compatible; MSIE 10.0; Windows Phone 8.0; Trident/6.0; IEMobile/10.0; ARM; Touch; NOKIA; Lumia 920; Vodafone)': {
      osname: 'Windows Phone'
      , windowsphone: true
      , osversion: '8.0'
      , msie: true
      , version: '10.0'
      , mobile: true
      , a: true
    }
    , 'Mozilla/5.0 (compatible; MSIE 10.0; Windows Phone 8.0; Trident/6.0; IEMobile/10.0; ARM; Touch; NOKIA; Lumia 920)': {
      osname: 'Windows Phone'
      , windowsphone: true
      , osversion: '8.0'
      , msie: true
      , version: '10.0'
      , mobile: true
      , a: true
    }
    , 'Mozilla/5.0 (compatible; MSIE 9.0; Windows Phone OS 7.5; Trident/5.0; IEMobile/9.0)': {
      osname: 'Windows Phone'
      , windowsphone: true
      , osversion: '7.5'
      , msie: true
      , version: '9.0'
      , mobile: true
      , c: true
    }
    , 'Mozilla/4.0 (compatible; MSIE 7.0; Windows Phone OS 7.0; Trident/3.1; IEMobile/7.0; Nokia;N70)': {
      osname: 'Windows Phone'
      , windowsphone: true
      , osversion: '7.0'
      , msie: true
      , version: '7.0'
      , mobile: true
      , c: true
    }
    , 'Mozilla/5.0 (Windows Phone 10.0; Android 4.2.1; DEVICE INFO) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/39.0.2171.71 Mobile Safari/537.36 Edge/12.0': {
      osname: 'Windows Phone'
      , windowsphone: true
      , osversion: '10.0'
      , msedge: true
      , version: '12.0'
      , mobile: true
      , a: true
    }
    , 'Mozilla/5.0 (Mobile; Windows Phone 8.1; Android 4.0; ARM; Trident/7.0; Touch; rv:11.0; IEMobile/11.0; Microsoft; Lumia 640 LTE) like iPhone OS 7_0_3 Mac OS X AppleWebKit/537 (KHTML, like Gecko) Mobile Safari/537': {
      osname: 'Windows Phone'
      , windowsphone: true
      , webkit: true
      , osversion: '8.1'
      , msie: true
      , version: '11.0'
      , mobile: true
      , a: true
    }
  }
  , WebOS: {
    'Mozilla/5.0 (hp-tablet; Linux; hpwOS/3.0.5; U; en-US) AppleWebKit/534.6 (KHTML, like Gecko) wOSBrowser/234.83 Safari/534.6 TouchPad/1.0': {
      osname: 'WebOS'
      , touchpad: true
      , webos: true
      , osversion: '3.0.5'
      , version: '234.83'
      , webkit: true
      , tablet: true
      , x: true
    }
    , 'Mozilla/5.0 (hp-tablet; Linux; hpwOS/3.0.2; U; en-US) AppleWebKit/534.6 (KHTML, like Gecko) wOSBrowser/234.40.1 Safari/534.6 TouchPad/1.0': {
      osname: 'WebOS'
      , touchpad: true
      , webos: true
      , osversion: '3.0.2'
      , version: '234.40'
      , webkit: true
      , tablet: true
      , x: true
    }
    , 'Mozilla/5.0 (hp-tablet; Linux; hpwOS/3.0.0; U; de-DE) AppleWebKit/534.6 (KHTML, like Gecko) wOSBrowser/233.70 Safari/534.6 TouchPad/1.0': {
      osname: 'WebOS'
      , touchpad: true
      , webos: true
      , osversion: '3.0.0'
      , version: '233.70'
      , webkit: true
      , tablet: true
      , x: true
    }
    , 'Mozilla/5.0 (Linux; webOS/2.1.2; U; xx-xx) AppleWebKit/534.6 (KHTML, like Gecko) webOSBrowser/221.11 Safari/534.6 Pre/3.0': {
      osname: 'WebOS'
      , webos: true
      , osversion: '2.1.2'
      , webkit: true
      , version: '221.11'
      , mobile: true
      , x: true
    }
    , 'Mozilla/5.0 (webOS/1.4.1.1; U; en-US) AppleWebKit/532.2 (KHTML, like Gecko) Version/1.0 Safari/532.2 Pre/1.0': {
      osname: 'WebOS'
      , webos: true
      , osversion: '1.4.1.1'
      , webkit: true
      , version: '1.0'
      , mobile: true
      , x: true
    }
    , 'Mozilla/5.0 (webOS/1.0; U; en-US) AppleWebKit/525.27.1 (KHTML, like Gecko) Version/1.0 Safari/525.27.1 Pre/1.0': {
      osname: 'WebOS'
      , webos: true
      , osversion: '1.0'
      , webkit: true
      , version: '1.0'
      , mobile: true
      , x: true
    }
  }
  , Android: {
    'Mozilla/5.0 (Linux; U; Android 4.4.2; de-de; Nexus 7 Build/KOT49H) AppleWebKit/537.16 (KHTML, like Gecko) Version/4.0 Mobile Safari/537.16': {
      osname: 'Android'
      , android: true
      , osversion: '4.4.2'
      , webkit: true
      , version: 4
      , tablet: true
      , x: true
    }
    , 'Mozilla/5.0 (Linux; U; Android 4.3; de-de; Galaxy Nexus Build/JWR66Y) AppleWebKit/534.30 (KHTML, like Gecko) Version/4.0 Mobile Safari/534.30': {
      osname: 'Android'
      , android: true
      , osversion: '4.3'
      , webkit: true
      , version: 4
      , mobile: true
      , x: true
    }
    , 'Mozilla/5.0 (Linux; U; Android 4.2; en-us; Nexus 10 Build/JVP15I) AppleWebKit/534.30 (KHTML, like Gecko) Version/4.0 Safari/534.30': {
      osname: 'Android'
      , android: true
      , osversion: '4.2'
      , webkit: true
      , version: 4
      , tablet: true
      , x: true
    }
    , 'Mozilla/5.0 (Linux; U; Android 3.2; de-de; A100 Build/HTJ85B) AppleWebKit/534.13 (KHTML, like Gecko) Version/4.0 Safari/534.13': {
      osname: 'Android'
      , android: true
      , osversion: '3.2'
      , webkit: true
      , version: 4
      , tablet: true
      , x: true
    }
    , 'Mozilla/5.0 (Linux; U; Android 2.3.4; en-us; T-Mobile G2 Build/GRJ22) AppleWebKit/533.1 (KHTML, like Gecko) Version/4.0 Mobile Safari/533.1': {
      osname: 'Android'
      , android: true
      , osversion: '2.3.4'
      , webkit: true
      , version: 4
      , mobile: true
      , x: true
    }
    , 'Mozilla/5.0 (Linux; U; Android 1.6; ar-us; SonyEricssonX10i Build/R2BA026) AppleWebKit/528.5+ (KHTML, like Gecko) Version/3.1.2 Mobile Safari/525.20.1': {
      osname: 'Android'
      , android: true
      , osversion: '1.6'
      , webkit: true
      , version: 3.1
      , mobile: true
      , x: true
    }
  }
  , Bada: {
    'Mozilla/5.0 (SAMSUNG; SAMSUNG-GT-S8500/S8500XPKJ1; U; Bada/2.0; it-it) AppleWebKit/534.20 (KHTML, like Gecko) Dolfin/3.0 WVGA SMM-MMS/1.2.0 OPN-B': {
      osname: 'Bada'
      , bada: true
      , osversion: '2.0'
      , webkit: true
      , version: '3.0'
      , mobile: true
      , x: true
    }
    , 'Mozilla/5.0 (SAMSUNG; SAMSUNG-GT-S8500/S8500XXJL2; U; Bada/1.2; de-de) AppleWebKit/533.1 (KHTML, like Gecko) Dolfin/2.2 Mobile WVGA SMM-MMS/1.2.0 OPN-B': {
      osname: 'Bada'
      , bada: true
      , osversion: '1.2'
      , webkit: true
      , version: '2.2'
      , mobile: true
      , x: true
    }
    , 'Mozilla/5.0 (SAMSUNG; SAMSUNG-GT-S8500/S8500XXJF4; U; Bada/1.0; fr-fr) AppleWebKit/533.1 (KHTML, like Gecko) Dolfin/2.0 Mobile WVGA SMM-MMS/1.2.0 OPN-B': {
      osname: 'Bada'
      , bada: true
      , osversion: '1.0'
      , webkit: true
      , version: '2.0'
      , mobile: true
      , x: true
    }
  }
  , Tizen: {
    'Mozilla/5.0 (Linux; Tizen 2.2; sdk) AppleWebKit/537.3 (KHTML, like Gecko) Version/2.2 Mobile Safari/537.3': {
      osname: 'Tizen'
      , tizen: true
      , osversion: '2.2'
      , webkit: true
      , version: '2.2'
      , mobile: true
      , x: true
    }
    , 'Mozilla/5.0 (Linux; Tizen 2.1; sdk) AppleWebKit/537.3 (KHTML, like Gecko) Version/2.1 Mobile Safari/537.3': {
      osname: 'Tizen'
      , tizen: true
      , osversion: '2.1'
      , webkit: true
      , version: '2.1'
      , mobile: true
      , x: true
    }
    , 'Mozilla/5.0 (Linux; U; Tizen 2.0; en-us) AppleWebKit/537.1 (KHTML, like Gecko) Mobile TizenBrowser/2.0': {
      osname: 'Tizen'
      , tizen: true
      , osversion: '2.0'
      , webkit: true
      , version: '2.0'
      , mobile: true
      , x: true
    }
    , 'Mozilla/5.0 (Linux; U; Tizen/1.0 like Android; en-us; AppleWebKit/534.46 (KHTML, like Gecko) Tizen Browser/1.0 Mobile': {
      osname: 'Tizen'
      , tizen: true
      , osversion: '1.0'
      , webkit: true
      , version: '1.0'
      , mobile: true
      , x: true
    }
    , 'Mozilla/5.0 (SAMSUNG; SAMSUNG-GT-i9500/1.0; U; Tizen/1.0 like Android; en-us) AppleWebKit/534.46 (KHTML, like Gecko) SLP Browser/1.0 Mobile': {
      osname: 'Tizen'
      , tizen: true
      , osversion: '1.0'
      , webkit: true
      , version: '1.0'
      , mobile: true
      , x: true
    }
  }
  , Sailfish: {
    'Mozilla/5.0 (Maemo; Linux; U; Jolla; Sailfish; Mobile; rv:26.0) Gecko/26.0 Firefox/26.0 SailfishBrowser/1.0 like Safari/538.1': {
      osname: 'Sailfish OS'
      , sailfish: true
      , gecko: true
      , version: '1.0'
      , mobile: true
      , x: true
    }
    , 'Mozilla/5.0 (Linux; U; Jolla; Sailfish; Mobile; rv:20.0) Gecko/20.0 Firefox/20.0 Sailfish Browser/1.0 like Safari/535.19': {
      osname: 'Sailfish OS'
      , sailfish: true
      , gecko: true
      , version: '1.0'
      , mobile: true
      , x: true
    }
  }
  , PhantomJS: {
    'Mozilla/5.0 (Macintosh; Intel Mac OS X) AppleWebKit/534.34 (KHTML, like Gecko) PhantomJS/1.5.0 Safari/534.34': {
      osname: 'macOS'
      , phantom: true
      , webkit: true
      , version: '1.5'
      , mac: true
      , x: true
    }
  }
  , Vivaldi: {
    'Mozilla/5.0 (Macintosh; Intel Mac OS X 10_11_1) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/45.0.2454.99 Safari/537.36 Vivaldi/1.0.303.52': {
      osname: 'macOS'
      , vivaldi: true
      , version: '1.0'
      , mac: true
      , osversion: '10.11.1'
      , blink: true
      , a: true
    }
    , 'Mozilla/5.0 (Windows NT 10.0; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/45.0.2454.99 Safari/537.36 Vivaldi/1.0.303.52': {
      osname: 'Windows'
      , vivaldi: true
      , version: '1.0'
      , windows: true
      , osversion: '10'
      , blink: true
      , a: true
    }
  }
  , Generic: {
    'Generic/2.15 libww': {
      version: '2.15'
      , x: true
    }
  }
  , Googlebot: {
    'Mozilla/5.0 (compatible; Googlebot/2.1; +http://www.google.com/bot.html)': {
      googlebot: true
      , version: '2.1'
      , x: true
    }
  }
  , 'UC Browser': {
    'Mozilla/5.0 (iPad; U; CPU OS 9 like Mac OS X; en-us; iPad4,4) AppleWebKit/534.46 (KHTML, like Gecko) UCBrowser/2.4.0.367 U3/1 Safari/7543.48.3': {
      osname: 'iOS'
      , ucbrowser: true
      , version: '2.4.0.367'
      , webkit: true
      , ipad: true
      , ios: true
      , osversion: 9
      , tablet: true
      , a: true
    }
    , 'Mozilla/5.0 (Linux; U; Android 4.1.2; en-us; SM-T210R Build/JZO54K) AppleWebKit/534.30 (KHTML, like Gecko) Version/4.0 Safari/534.30 UCBrowser/2.3.2.300': {
      osname: 'Android'
      , android: true
      , osversion: '4.1.2'
      , tablet: true
      , ucbrowser: true
      , version: '2.3.2.300'
      , webkit: true
      , x: true
    }
    , 'Mozilla/5.0 (iPhone; CPU iPhone OS 9_3_1 like Mac OS X; en-US) AppleWebKit/537.51.1 (KHTML, like Gecko) Mobile/13E238 UCBrowser/10.7.0.782 Mobile': {
      osname: 'iOS'
      , iphone: true
      , ios: true
      , mobile: true
      , osversion: '9.3.1'
      , version: '10.7.0.782'
      , webkit: true
      , a: true
      , ucbrowser: true
    }
  }
  , QupZilla: {
    'Mozilla/5.0 (Macintosh; Intel Mac OS X) AppleWebKit/538.1 (KHTML, like Gecko) QupZilla/1.8.2 Safari/538.1': {
      osname: 'macOS'
      , mac: true
      , qupzilla: true
      , version: '1.8.2'
      , webkit: true
      , x: true
    }
    , 'Mozilla/5.0 (Windows NT 6.2; WOW64) AppleWebKit/537.21 (KHTML, like Gecko) QupZilla/1.8.6 Safari/537.21': {
      osname: 'Windows'
      , windows: true
      , osversion: '8'
      , qupzilla: true
      , version: '1.8.6'
      , webkit: true
      , x: true
    }
  }
  , Maxthon: {
    'Mozilla/5.0 (iPhone; CPU iPhone OS 9_3_1 like Mac OS X) AppleWebKit/601.1.46 (KHTML, like Gecko) Version/9.0 Mobile/13E238 Safari/601.1 MXiOS/4.8.6.59': {
      osname: 'iOS'
      , mobile: true
      , ios: true
      , iphone: true
      , version: '4.8.6.59'
      , osversion: '9.3.1'
      , webkit: true
      , maxthon: true
      , a: true
    }
  }
  , Epiphany: {
    'Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/602.1 (KHTML, like Gecko) Version/8.0 Safari/602.1 Debian/buildd-unstable (3.18.5-1) Epiphany/3.18.5': {
      osname: 'Linux'
      , epiphany: true
      , x: true
      , webkit: true
      , version: '3.18.5'
      , linux: true
    }
  }
  , Puffin: {
    'Mozilla/5.0 (X11; U; Linux x86_64; zh-TW) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/30.0.1599.114 Safari/537.36 Puffin/3.7.0IT': {
      osname: 'Linux'
      , linux: true
      , blink: true
      , puffin: true
      , version: '3.7'
      , x: true
    }
  }
  , Sleipnir: {
    'Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/39.0.2171.95 Safari/537.36 Sleipnir/6.1.4': {
      osname: 'Windows'
      , sleipnir: true
      , blink: true
      , windows: true
      , osversion: '7'
      , x: true
      , version: '6.1.4'
    }
  }
  , 'K-Meleon': {
    'Mozilla/5.0 (Windows NT 6.1; WOW64; rv:24.0) Gecko/20140105 Firefox/24.0 K-Meleon/74.0': {
      name: 'K-Meleon'
      , osname: 'Windows'
      , windows: true
      , osversion: '7'
      , kMeleon: true
      , version: '74.0'
      , gecko: true
      , x: true
    }
  }
  , Chromium: {
    'Mozilla/5.0 (Linux; Ubuntu 14.04 like Android 4.4) AppleWebKit/537.36 Chromium/35.0.1870.2 Mobile Safari/537.36': {
      osname: 'Linux'
      , mobile: true
      , linux: true
      , chromium: true
      , version: '35.0'
      , blink: true
      , a: true
    }
    , 'Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Ubuntu Chromium/50.0.2661.102 Chrome/50.0.2661.102 Safari/537.36': {
      osname: 'Linux'
      , chromium: true
      , a: true
      , version: '50.0'
      , linux: true
      , blink: true
    }
  }
  , SlimerJS: {
    'Mozilla/5.0 (X11; Linux x86_64; rv:21.0) Gecko/20100101 SlimerJS/0.7': {
      osname: 'Linux'
      , linux: true
      , slimer: true
      , version: '0.7'
      , gecko: true
      , x: true
    }
  }
  , 'Samsung Internet for Android': {
    'Mozilla/5.0 (Linux; Android 5.0.2; SAMSUNG SM-G925F Build/LRX22G) AppleWebKit/537.36 (KHTML, like Gecko) SamsungBrowser/4.0 Chrome/44.0.2403.133 Mobile Safari/537.36': {
      osname: 'Android'
      , mobile: true
      , android: true
      , osversion: '5.0.2'
      , samsungBrowser: true
      , version: '4.0'
      , a: true
      , blink: true
    }
  }
}



var AndroidOSVersionRegex = new RegExp(/android[ \/-](\d+(\.\d+)*)/),
AndroidRegex = new RegExp(/android/),
BadaOSVersionRegex = new RegExp(/bada\/(\d+(\.\d+)*)/),
BadaRegex = new RegExp(/bada/),
BadaVersionRegex = new RegExp(/dolfin\/(\d+(\.\d+)?)/),
BlackberryOSVersionRegex = new RegExp(/rim\stablet\sos\s(\d+(\.\d+)*)/),
BlackberryRegex = new RegExp(/blackberry|\bbb\d+/),
BlackberryVersionRegex = new RegExp(/blackberry[\d]+\/(\d+(\.\d+)?)/),
BlinkRegex = new RegExp(/(apple)?webkit\/537\.36/),
ChromiumRegex = new RegExp(/chromium/),
ChromiumVersionRegex = new RegExp(/(?:chromium)[\s\/](\d+(?:\.\d+)?)/),
ChromeOSRegex = new RegExp(/CrOS/),
ChromeOSVersionRegex = new RegExp(/(?:chrome|crios|crmo)\/(\d+(\.\d+)?)/),
ChromeRegex = new RegExp(/chrome|crios|crmo/),
ChromeVersionRegex = new RegExp(/(?:chrome|crios|crmo)\/(\d+(\.\d+)?)/),
CoastRegex = new RegExp(/coast/),
CoastVersionRegex = new RegExp(/(?:coast)[\s\/](\d+(\.\d+)?)/),
EdgeRegex = new RegExp(/chrome.+? edge/),
EdgeVersionRegex = new RegExp(/edge\/(\d+(\.\d+)?)/),
EpiphanyRegex = new RegExp(/epiphany/),
EpiphanyVersionRegex = new RegExp(/(?:epiphany)[\s\/](\d+(?:\.\d+)+)/),
FirefoxRegex = new RegExp(/firefox|iceweasel|fxios/),
FirefoxOSRegex = new RegExp(/\((mobile|tablet),[^\)]*rv:[\d\.]+\)/),
FirefoxVersionRegex = new RegExp(/(?:firefox|iceweasel|fxios)[ \/](\d+(\.\d+)?)/),
GeckoRegex = new RegExp(/gecko/),
GeckoVersionRegex = new RegExp(/gecko\/(\d+(\.\d+)?)/),
GooglebotRegex = new RegExp(/googlebot/),
GooglebotVersionRegex = new RegExp(/googlebot\/(\d+(\.\d+))/),
IEMobileVersionRegex = new RegExp(/iemobile\/(\d+(\.\d+)?)/),
InternetExplorerRegex = new RegExp(/msie|trident/),
InternetExplorerVersionRegex = new RegExp(/(?:msie |rv:)(\d+(\.\d+)?)/),
iOSDeviceRegex = new RegExp(/(ipod|iphone|ipad)/),
iOSOSVersionRegex = new RegExp(/os (\d+([_\s]\d+)*) like mac os x/),
iOSOSVersionReplaceRegex = new RegExp(/[_\s]/),
KMeleonRegex = new RegExp(/k-meleon/),
KMeleonVersionRegex = new RegExp(/(?:k-meleon)[\s\/](\d+(?:\.\d+)+)/),
LikeAndroidRegex = new RegExp(/like android/),
LinuxRegex = new RegExp(/linux/),
MacRegex = new RegExp(/macintosh/),
macOSOSVersionRegex = new RegExp(/Mac OS X (\d+([_\.\s]\d+)*)/),
macOSOSVersionReplaceRegex = new RegExp(/Mac OS X (\d+([_\.\s]\d+)*)/),
MobileRegex = new RegExp(/[^-]mobi/),
MaxthonRegex = new RegExp(/mxios/),
MaxthonVersionRegex = new RegExp(/(?:mxios)[\s\/](\d+(?:\.\d+)+)/),
NexusMobileRegex = new RegExp(/nexus\s*[0-6]\s*/),
NexusTabletRegex = new RegExp(/nexus\s*[0-9]+/),
NewOperaRegex = new RegExp(/opr\/|opios/),
NewOperaVersionRegex = new RegExp(/(?:opr|opios)[\s\/](\d+(\.\d+)?)/),
OldOperaRegex = new RegExp(/opera/),
OldOperaVersionRegex = new RegExp(/(?:opera|opr|opios)[\s\/](\d+(\.\d+)?)/),
PuffinRegex = new RegExp(/puffin/),
PuffinVersionRegex = new RegExp(/(?:puffin)[\s\/](\d+(?:\.\d+)?)/),
PhantomJsRegex = new RegExp(/phantom/),
PhantomJsVersionRegex = new RegExp(/phantomjs\/(\d+(\.\d+)?)/),
QupZillaRegex = new RegExp(/qupzilla/),
QupZillaVersionRegex = new RegExp(/(?:qupzilla)[\s\/](\d+(?:\.\d+)+)/),
RimRegex = new RegExp(/rim\stablet/),
SafariRegex = new RegExp(/safari|applewebkit/),
SailfishRegex = new RegExp(/sailfish/),
SailfishVersionRegex = new RegExp(/sailfish\s?browser\/(\d+(\.\d+)?)/),
SamsungBrowserRegex = new RegExp(/SamsungBrowser/),
SamsungBrowserVersionRegex = new RegExp(/(?:SamsungBrowser)[\s\/](\d+(\.\d+)?)/),
SeaMonkeyRegex = new RegExp(/seamonkey\//),
SeaMonkeyVersionRegex = new RegExp(/seamonkey\/(\d+(\.\d+)?)/),
SilkRegex = new RegExp(/silk/),
SilkVersionRegex = new RegExp(/silk\/(\d+(\.\d+)?)/),
SleipnirRegex = new RegExp(/sleipnir/),
SleipnirVersionRegex = new RegExp(/(?:sleipnir)[\s\/](\d+(?:\.\d+)+)/),
SlimerJsRegex = new RegExp(/slimerjs/),
SlimerJsVersionRegex = new RegExp(/slimerjs\/(\d+(\.\d+)?)/),
TabletRegex = new RegExp(/tablet/),
TabletPCRegex = new RegExp(/tablet pc/),
TizenOSVersionRegex = new RegExp(/tizen[\/\s](\d+(\.\d+)*)/),
TizenRegex = new RegExp(/tizen/),
TizenVersionRegex = new RegExp(/(?:tizen\s?)?browser\/(\d+(\.\d+)?)/),
TouchPadRegex = new RegExp(/touchpad\//),
UcBrowserRegex = new RegExp(/ucbrowser/),
UcBrowserVersionRegex = new RegExp(/(?:ucbrowser)[\s\/](\d+(?:\.\d+)+)/),
UnknownRegex = new RegExp(/^(.*)\/(.*) /),
VersionIdentifierRegex = new RegExp(/version\/(\d+(\.\d+)?)/),
VivaldiRegex = new RegExp(/vivaldi/),
VivaldiVersionRegex = new RegExp(/vivaldi\/(\d+(\.\d+)?)/),
WebkitRegex = new RegExp(/(apple)?webkit/),
WebOSOSVersionRegex = new RegExp(/(?:web|hpw)os\/(\d+(\.\d+)*)/),
WebOSRegex = new RegExp(/(web|hpw)os/),
WebOSVersionRegex = new RegExp(/w(?:eb)?osbrowser\/(\d+(\.\d+)?)/),
WindowsOSVersionRegex = new RegExp(/Windows ((NT|XP)( \d\d?.\d)?)/),
WindowsPhoneOSVersionRegex = new RegExp(/windows phone (?:os)?\s?(\d+(\.\d+)*)/),
WindowsPhoneRegex = new RegExp(/windows phone/),
WindowsRegex = new RegExp(/windows/),
XboxRegex = new RegExp(/xbox/),
YandexBrowserRegex = new RegExp(/yabrowser/),
YandexBrowserVersionRegex = new RegExp(/(?:yabrowser)[\s\/](\d+(\.\d+)?)/)
;


  function AgentFinder(ua) {

      Opera = false,
      SamsungBrowser = false,
      Coast = false,
      YandexBrowser = false,
      UcBrowser = false,
      Maxthon = false,
      Epiphany = false,
      Puffin = false,
      Sleipnir = false,
      KMeleon = false,
      WindowsPhone = false,
      MsIE = false,
      ChromeOS = false,
      ChromeBook = false,
      MsEdge = false,
      Vivaldi = false,
      Sailfish = false,
      SeaMonkey = false,
      Firefox = false,
      FirefoxOS = false,
      Silk = false,
      Phantom = false,
      Slimer = false,
      Blackberry = false,
      WebOS = false,
      TouchPad = false,
      Bada = false,
      Tizen = false,
      QupZilla = false,
      Chromium = false,
      Chrome = false,
      Safari = false,
      Googlebot = false,
      Blink = false,
      Webkit = false,
      Gecko = false,
      Android = false,
      iPhone = false,
      iPad = false,
      iPod = false,
      iOS = false,
      Mac = false,
      Xbox = false,
      Windows = false,
      Linux = false,
      Tablet = false,
      Mobile = false,
      Xbox = false;

      uaToLower = ua ? ua.toLowerCase() : '';

      this.getFirstMatch=function(regex){
          match = uaToLower.match(regex);
          if (match && match.length > 0)
              return match[0];
          return '';
      };

      this.getSecondMatch=function(regex){
          match = uaToLower.match(regex);
          if (match && match.length > 1)
              return match[1];
          return '';
      };

      iosDevice = this.getFirstMatch(iOSDeviceRegex),
      likeAndroid = uaToLower.match(LikeAndroidRegex),
      android = !likeAndroid && uaToLower.match(AndroidRegex),
      nexusMobile = uaToLower.match(NexusMobileRegex),
      nexusTablet = !nexusMobile && uaToLower.match(NexusTabletRegex),
      chromeOS = uaToLower.match(ChromeOSRegex),
      silk = uaToLower.match(SilkRegex),
      sailfish = uaToLower.match(SailfishRegex),
      tizen = uaToLower.match(TizenRegex),
      webos = uaToLower.match(WebOSRegex),
      windowsPhone = uaToLower.match(WindowsPhoneRegex),
      samsungBrowser = uaToLower.match(SamsungBrowserRegex),
      windows = !windowsPhone && uaToLower.match(WindowsRegex),
      mac = !iosDevice && !silk && uaToLower.match(MacRegex),
      linux = !android && !sailfish && !tizen && !webos && uaToLower.match(LinuxRegex),
      edgeVersion = this.getFirstMatch(EdgeVersionRegex),
      tablet = uaToLower.match(TabletRegex) && !uaToLower.match(TabletPCRegex),
      mobile = !tablet && uaToLower.match(MobileRegex),
      xbox = uaToLower.match(XboxRegex);

      versionIdentifier = this.getFirstMatch(VersionIdentifierRegex);

      if (uaToLower.match(OldOperaRegex))
      {
          Name = "Opera";
          Opera = true;
          Version = versionIdentifier || this.getFirstMatch(OldOperaVersionRegex)
      }
      else if (uaToLower.match(NewOperaRegex))
      {
          Name = "Opera";
          Opera = true;
          Version = this.getFirstMatch(NewOperaVersionRegex) || versionIdentifier;
      }
      else if (uaToLower.match(SamsungBrowserRegex))
      {
          Name = "Samsung Internet for Android";
          SamsungBrowser = true;
          Version = versionIdentifier || this.getFirstMatch(SamsungBrowserVersionRegex)
      }
      else if (uaToLower.match(CoastRegex))
      {
          Name = "Opera Coast";
          Coast = true;
          Version = versionIdentifier || this.getFirstMatch(CoastVersionRegex)
      }
      else if (uaToLower.match(YandexBrowserRegex))
      {
          Name = "Yandex Browser";
          YandexBrowser = true;
          Version = versionIdentifier || this.getFirstMatch(YandexBrowserVersionRegex)
      }
      else if (uaToLower.match(UcBrowserRegex))
      {
          Name = "UC Browser";
          UcBrowser = true;
          Version = this.getFirstMatch(UcBrowserVersionRegex)
      }
      else if (uaToLower.match(MaxthonRegex))
      {
          Name = "Maxthon";
          Maxthon = true;
          Version = this.getFirstMatch(MaxthonVersionRegex)
      }
      else if (uaToLower.match(EpiphanyRegex))
      {
          Name = "Epiphany";
          Epiphany = true;
          Version = this.getFirstMatch(EpiphanyVersionRegex)
      }
      else if (uaToLower.match(PuffinRegex))
      {
          Name = "Puffin";
          Puffin = true;
          Version = this.getFirstMatch(PuffinVersionRegex)
      }
      else if (uaToLower.match(SleipnirRegex))
      {
          Name = "Sleipnir";
          Sleipnir = true;
          Version = this.getFirstMatch(SleipnirVersionRegex)
      }
      else if (uaToLower.match(KMeleonRegex))
      {
          Name = "K-Meleon";
          KMeleon = true;
          Version = this.getFirstMatch(KMeleonVersionRegex)
      }
      else if (windowsPhone)
      {
          Name = "Windows Phone";
          WindowsPhone = true;
          if (edgeVersion)
          {
              MsEdge = true;
              Version = edgeVersion;
          }
          else
          {
              MsIE = true;
              Version = this.getFirstMatch(IEMobileVersionRegex)
          }
      }
      else if (uaToLower.match(InternetExplorerRegex))
      {
          Name = "Internet Explorer";
          MsIE = true;
          Version = this.getFirstMatch(InternetExplorerVersionRegex)
      }
      else if (chromeOS)
      {
          Name = "Chrome";
          ChromeOS = true;
          ChromeBook = true;
          Chrome = true;
          Version = this.getFirstMatch(ChromeOSVersionRegex)
      }
      else if (uaToLower.match(EdgeRegex))
      {
          Name = "Microsoft Edge";
          MsEdge = true;
          Version = edgeVersion;
      }
      else if (uaToLower.match(VivaldiRegex))
      {
          Name = "Vivaldi";
          Vivaldi = true;
          Version = this.getFirstMatch(VivaldiVersionRegex) || versionIdentifier;
      }
      else if (sailfish)
      {
          Name = "Sailfish";
          Sailfish = true;
          Version = this.getFirstMatch(SailfishVersionRegex)
      }
      else if (uaToLower.match(SeaMonkeyRegex))
      {
          Name = "SeaMonkey";
          SeaMonkey = true;
          Version = this.getFirstMatch(SeaMonkeyVersionRegex)
      }
      else if (uaToLower.match(FirefoxRegex))
      {
          Name = "Firefox";
          Firefox = true;
          Version = this.getFirstMatch(FirefoxVersionRegex)
          if (uaToLower.match(FirefoxOSRegex))
              FirefoxOS = true;
      }
      else if (silk)
      {
          Name = "Amazon Silk";
          Silk = true;
          Version = this.getFirstMatch(SilkVersionRegex)
      }
      else if (uaToLower.match(PhantomJsRegex))
      {
          Name = "PhantomJS";
          Phantom = true;
          Version = this.getFirstMatch(PhantomJsVersionRegex)
      }
      else if (uaToLower.match(SlimerJsRegex))
      {
          Name = "SlimerJS";
          Slimer = true;
          Version = this.getFirstMatch(SlimerJsVersionRegex)
      }
      else if (uaToLower.match(BlackberryRegex) || uaToLower.match(RimRegex))
      {
          Name = "Blackberry";
          Blackberry = true;
          Version = versionIdentifier || this.getFirstMatch(BlackberryVersionRegex)
      }
      else if (webos)
      {
          Name = "WebOS";
          WebOS = true;
          Version = versionIdentifier || this.getFirstMatch(WebOSVersionRegex)
          if (uaToLower.match(TouchPadRegex))
              TouchPad = true;
      }
      else if (uaToLower.match(BadaRegex))
      {
          Name = "Bada";
          Bada = true;
          Version = this.getFirstMatch(BadaVersionRegex)
      }
      else if (tizen)
      {
          Name = "Tizen";
          Tizen = true;
          Version = this.getFirstMatch(TizenVersionRegex) || versionIdentifier;
      }
      else if (uaToLower.match(QupZillaRegex))
      {
          Name = "QupZilla";
          QupZilla = true;
          Version = this.getFirstMatch(QupZillaVersionRegex) || versionIdentifier;
      }
      else if (uaToLower.match(ChromiumRegex))
      {
          Name = "Chromium";
          Chromium = true;
          Version = this.getFirstMatch(ChromiumVersionRegex) || versionIdentifier;
      }
      else if (uaToLower.match(ChromeRegex))
      {
          Name = "Chrome";
          Chrome = true;
          Version = this.getFirstMatch(ChromeVersionRegex)
      }
      else if (android)
      {
          Name = "Android";
          Version = versionIdentifier;
      }
      else if (uaToLower.match(SafariRegex))
      {
          Name = "Safari";
          Safari = true;
          if (versionIdentifier)
              Version = versionIdentifier;
      }
      else if (iosDevice)
      {
          Name = iosDevice.toLowerCase() ==  "iphone"  ? "iPhone" : iosDevice.toLowerCase() ==  "ipad"  ? "iPad" : "iPod";
          if (versionIdentifier)
              Version = versionIdentifier;
      }
      else if (uaToLower.match(GooglebotRegex))
      {
          Name = "Googlebot";
          Googlebot = true;
          Version = this.getFirstMatch(GooglebotVersionRegex) || versionIdentifier;
      }
      else
      {
          Name = this.getFirstMatch(UnknownRegex)
          Version = this.getSecondMatch(UnknownRegex)
      }

      if (!MsEdge && uaToLower.match(WebkitRegex))
      {
          if (uaToLower.match(BlinkRegex))
          {
              Name = Name || "Blink";
              Blink = true;
          }
          else
          {
              Name = Name || "Webkit";
              Webkit = true;
          }
          if (!Version && versionIdentifier)
              Version = versionIdentifier;
      }
      else if (!Opera && uaToLower.match(GeckoRegex))
      {
          Name = Name || "Gecko";
          Gecko = true;
          Version = Version || this.getFirstMatch(GeckoVersionRegex)
      }

      if (!WindowsPhone && !MsEdge && (android || Silk))
          Android = true;
      else if (!WindowsPhone && !MsEdge && iosDevice)
      {
          if (iosDevice.toLowerCase() ==  "iphone" )
              iPhone = true;
          else if (iosDevice.toLowerCase() ==  "ipad" )
              iPad = true;
          else
              iPod = true;
          iOS = true;
      }
      else if (mac)
          Mac = true;
      else if (xbox)
          Xbox = true;
      else if (windows)
          Windows = true;
      else if (linux)
          Linux = true;

      osVersion = '';

      if (Windows)
      {
          osVersion = this.getFirstMatch(WindowsOSVersionRegex)
          if (osVersion == "NT")
              osVersion = "NT";
          else if (osVersion == "XP")
              osVersion = "XP";
          else if (osVersion == "NT 5.0")
              osVersion = "2000";
          else if (osVersion == "NT 5.1")
              osVersion = "XP";
          else if (osVersion == "NT 5.2")
              osVersion = "2003";
          else if (osVersion == "NT 6.0")
              osVersion = "Vista";
          else if (osVersion == "NT 6.1")
              osVersion = "7";
          else if (osVersion == "NT 6.2")
              osVersion = "8";
          else if (osVersion == "NT 6.3")
              osVersion = "8.1";
          else if (osVersion == "NT 10.0")
              osVersion = "10";
          else
              osVersion = null;
      }
      else if (WindowsPhone)
          osVersion = this.getFirstMatch(WindowsPhoneOSVersionRegex)
      else if (Mac)
          osVersion = macOSOSVersionReplaceRegex.toString().replace(this.getFirstMatch(macOSOSVersionRegex), ".")
      else if (iosDevice)
      {
          osVersion = this.getFirstMatch(iOSOSVersionRegex)
          osVersion = iOSOSVersionReplaceRegex.toString().replace(osVersion, ".")
      }
      else if (android)
          osVersion = this.getFirstMatch(AndroidOSVersionRegex)
      else if (WebOS)
          osVersion = this.getFirstMatch(WebOSOSVersionRegex)
      else if (Blackberry)
          osVersion = this.getFirstMatch(BlackberryOSVersionRegex)
      else if (Bada)
          osVersion = this.getFirstMatch(BadaOSVersionRegex)
      else if (Tizen)
          osVersion = this.getFirstMatch(TizenOSVersionRegex)
      if (osVersion)
          OSVersion = osVersion;

      osMajorVersion = Windows ? null : osVersion.split('.')[0];

      if (tablet ||
          nexusTablet ||
          iosDevice.toLowerCase() ==  "ipad"  ||
          (android && (osMajorVersion == 3 || (osMajorVersion >= 4 && !mobile))) ||
          Silk)
          Tablet = true;
      else if (mobile ||
          iosDevice.toLowerCase() ==  "iphone"  ||
          iosDevice.toLowerCase() ==  "ipod"  ||
          android ||
          nexusMobile ||
          Blackberry ||
          WebOS ||
          Bada)
          Mobile = true;

      versionFloat = Version

      if (MsEdge ||
          (MsIE && versionFloat >= 10) ||
          (YandexBrowser && versionFloat >= 15) ||
          (Vivaldi && versionFloat >= 1.0) ||
          (Chrome && versionFloat >= 20) ||
          (SamsungBrowser && versionFloat >= 4) ||
          (Firefox && versionFloat >= 20) ||
          (Safari && versionFloat >= 6) ||
          (Opera && versionFloat >= 10.0) ||
          (iOS && OSVersion && parseInt(OSVersion.split('.')[0]) >= 6) ||
          (Blackberry && versionFloat >= 10.1) ||
          (Chromium && versionFloat >= 20))
          A = true;
      else if ((MsIE && versionFloat < 10) ||
          (Chrome && versionFloat < 20) ||
          (Firefox && versionFloat < 20.0) ||
          (Safari && versionFloat < 6) ||
          (Opera && versionFloat < 10.0) ||
          (iOS && OSVersion && parseInt(OSVersion.split('.')[0]) < 6) ||
          (Chromium && versionFloat < 20))
          C = true;
      else
          X = true;


        this.obj = {
          name: Name,
          version: Version,
        }


        // ------------------------------------------------
        if (Opera) this.obj.Opera = true;
        if (SamsungBrowser) this.obj.SamsungBrowser = true;
        if (Coast) this.obj.Coast = true;
        if (YandexBrowser) this.obj.YandexBrowser = true;
        if (UcBrowser) this.obj.UcBrowser = true;
        if (Maxthon) this.obj.Maxthon = true;
        if (Epiphany) this.obj.Epiphany = true;
        if (Puffin) this.obj.Puffin = true;
        if (Sleipnir) this.obj.Sleipnir = true;
        if (KMeleon) this.obj.KMeleon = true;
        if (WindowsPhone) this.obj.WindowsPhone = true;
        if (MsIE) this.obj.MsIE = true;
        if (ChromeOS) this.obj.ChromeOS = true;
        if (ChromeBook) this.obj.ChromeBook = true;
        if (MsEdge) this.obj.MsEdge = true;
        if (Vivaldi) this.obj.Vivaldi = true;
        if (Sailfish) this.obj.Sailfish = true;
        if (SeaMonkey) this.obj.SeaMonkey = true;
        if (Firefox) this.obj.Firefox = true;
        if (FirefoxOS) this.obj.FirefoxOS = true;
        if (Silk) this.obj.Silk = true;
        if (Phantom) this.obj.Phantom = true;
        if (Slimer) this.obj.Slimer = true;
        if (Blackberry) this.obj.Blackberry = true;
        if (WebOS) this.obj.WebOS = true;
        if (TouchPad) this.obj.TouchPad = true;
        if (Bada) this.obj.Bada = true;
        if (Tizen) this.obj.Tizen = true;
        if (QupZilla) this.obj.QupZilla = true;
        if (Chromium) this.obj.Chromium = true;
        if (Chrome) this.obj.Chrome = true;
        if (Safari) this.obj.Safari = true;
        if (Googlebot) this.obj.Googlebot = true;
        if (Blink) this.obj.Blink = true;
        if (Webkit) this.obj.Webkit = true;
        if (Gecko) this.obj.Gecko = true;

        if (Android) this.obj.Android = true;
        if (iPhone) this.obj.iPhone = true;
        if (iPad) this.obj.iPad = true;
        if (iPod) this.obj.iPod = true;

        if (iOS) this.obj.iOS = true;
        if (Mac) this.obj.Mac = true;
        if (Xbox) this.obj.Xbox = true;
        if (Windows) this.obj.Windows = true;
        if (Linux) this.obj.Linux = true;

        if (Tablet) this.obj.Tablet = true;
        if (Mobile) this.obj.Mobile = true;
        if (Xbox) this.obj.Xbox = true;

        // ------------------------------------------------
        if (iosDevice) this.obj.iosDevice = true;
        if (likeAndroid) this.obj.likeAndroid = true;
        if (android) this.obj.android = true;
        if (nexusMobile) this.obj.nexusMobile = true;
        if (nexusTablet) this.obj.nexusTablet = true;
        if (chromeOS) this.obj.chromeOS = true;
        if (silk) this.obj.silk = true;
        if (sailfish) this.obj.sailfish = true;
        if (tizen) this.obj.tizen = true;
        if (webos) this.obj.webos = true;
        if (windowsPhone) this.obj.windowsPhone = true;
        if (samsungBrowser) this.obj.samsungBrowser = true;
        if (windows) this.obj.windows = true;
        if (mac) this.obj.mac = true;
        if (linux) this.obj.linux = true;
        if (versionIdentifier) this.obj.versionIdentifier = true;
        if (tablet) this.obj.tablet = true;
        if (mobile) this.obj.mobile = true;
        if (xbox) this.obj.xbox = true;
        // ------------------------------------------------


        // Other browsers
        if (Coast) this.obj.browser = 'Coast';
        if (SamsungBrowser) this.obj.browser = 'SamsungBrowser';
        if (YandexBrowser) this.obj.browser = 'YandexBrowser';
        if (UcBrowser) this.obj.browser = 'UcBrowser';
        if (Maxthon) this.obj.browser = 'Maxthon';
        if (Epiphany) this.obj.browser = 'Epiphany';
        if (Puffin) this.obj.browser = 'Puffin';
        if (Sleipnir) this.obj.browser = 'Sleipnir';
        if (KMeleon) this.obj.browser = 'KMeleon';
        if (MsIE) this.obj.browser = 'MsIE';
        if (MsEdge) this.obj.browser = 'MsEdge';
        if (Vivaldi) this.obj.browser = 'Vivaldi';
        if (Sailfish) this.obj.browser = 'Sailfish';
        if (SeaMonkey) this.obj.browser = 'SeaMonkey';
        if (Silk) this.obj.browser = 'Silk';
        if (Phantom) this.obj.browser = 'Phantom';
        if (Slimer) this.obj.browser = 'Slimer';
        if (WebOS) this.obj.browser = 'WebOS';
        if (TouchPad) this.obj.browser = 'TouchPad';
        if (Bada) this.obj.browser = 'Bada';
        if (Tizen) this.obj.browser = 'Tizen';
        if (QupZilla) this.obj.browser = 'QupZilla';
        if (Chromium) this.obj.browser = 'Chromium';
        if (Googlebot) this.obj.browser = 'Googlebot';
        if (Blink) this.obj.browser = 'Blink';
        if (Webkit) this.obj.browser = 'Webkit';
        if (Gecko) this.obj.browser = 'Gecko';

        // Popular browsers
        if (Opera) this.obj.browser = 'Opera';
        if (Firefox) this.obj.browser = 'Firefox';
        if (Chrome) this.obj.browser = 'Chrome';
        if (Safari) this.obj.browser = 'Safari';
        if (MsEdge) this.obj.browser = 'Edge';
        if (MsIE) this.obj.browser = 'Internet Explorer';


        // Mobile Device
        if (Blackberry) this.obj.mobileDevice = 'Blackberry';
        if (WindowsPhone) this.obj.mobileDevice = 'WindowsPhone';
        if (Android) this.obj.mobileDevice = 'Android';
        if (iPhone) this.obj.mobileDevice = 'iPhone';
        if (iPad) this.obj.mobileDevice = 'iPad';
        if (iPod) this.obj.mobileDevice = 'iPod';

        // General Device Identifier
        if (Tablet) this.obj.device = 'Tablet';
        if (Mobile) this.obj.device = 'Mobile';
        if (Xbox) this.obj.device = 'Xbox';
        if (!Tablet && !Mobile && !Xbox) this.obj.device = 'Desktop';

        // OS
        if (ChromeOS) this.obj.os = 'ChromeOS';
        if (ChromeBook) this.obj.os = 'ChromeBook';
        if (FirefoxOS) this.obj.os = 'FirefoxOS';
        if (Android) this.obj.os = 'Android';
        if (iOS || iPhone || iPad || iPod) this.obj.os = 'iOS';
        if (Mac) this.obj.os = 'Mac';
        if (Xbox) this.obj.os = 'Xbox';
        if (Windows) this.obj.os = 'Windows';
        if (Linux) this.obj.os = 'Linux';


        // if (A) this.obj.browser = 'A';
        // if (C) this.obj.browser = 'C';
        // if (X) this.obj.browser = 'X';

    return this.obj;

  }


